...
$('apply-link').addEventListener('click',()=>{const value=$('element-link').value.trim();if(!safeLink(value)){toast('Use an https://, mailto:, tel: or #section link.');return;}editor.getSelected()?.addAttributes({href:value||'#'});markDirty();toast('Link updated.');});
...
function applyTheme(style,headFont){const wrapper=editor.getWrapper();wrapper.setStyle({...wrapper.getStyle(),...style});if(headFont)editor.Css.setRule('h1, h2, h3, h4',{'font-family':headFont});markDirty();}
...
function showStart(kind='new'){document.querySelector(`[name="project-kind"][value="${kind}"]`).checked=true;$('close-start').hidden=!started;$('start-name').value='';$('start-brief').value='';$('start-file').value='';$('start-error').textContent='';updateStartMode();refreshRestore();$('start-dialog').showModal();}
...
async function readProject(file){if(!file)throw new Error('Choose an HTML website or an Oakline project file.');if(file.size>15*1024*1024)throw new Error('Choose a project smaller than 15 MB.');const source=await file.text();if(/\.json$/i.test(file.name)){let root;try{root=JSON.parse(source);}catch{throw new Error('This JSON file could not be read.');}if(root.format==='oakline-canvas-project'&&root.version===2&&typeof root.html==='string'&&typeof root.css==='string'){if(root.html.length+root.css.length>14*1024*1024)throw new Error('Project content is too large.');return{content:cleanDocument(root.html,root.css),name:typeof root.name==='string'?root.name.slice(0,80):'',brief:typeof root.brief==='string'?root.brief.slice(0,4000):''};}if(root.format==='oakline-studio-project'&&root.schemaVersion===1)return{content:legacyProject(root.project),name:String(root.project.brand||'Imported website').slice(0,80),brief:'',legacy:true};throw new Error('Choose an Oakline project file, or import an HTML website.');}if(!/\.(html?|htm)$/i.test(file.name))throw new Error('Choose an HTML or Oakline JSON file.');return{content:cleanDocument(source),name:'',brief:''};}
function openProject(content,{name,brief,kind,message}){meta={name,brief,kind};loadContent(content,{resetHistory:true});$('project-name').value=meta.name;$('project-brief').value=meta.brief;$('project-mode').textContent=kind==='new'?'New website':'Editing website';$('ai-results').replaceChildren();suggestionRun++;started=true;dirty=false;$('start-dialog').close();switchTab('add');toast(message);}
$('start-form').addEventListener('submit',async event=>{event.preventDefault();$('start-error').textContent='';const kind=document.querySelector('[name="project-kind"]:checked').value;try{const imported=kind==='existing'?await readProject($('start-file').files[0]):null;if(started&&dirty&&!confirm('Open a different website? Your current work is autosaved in this browser only until you start something new — download an editable project first if you want to keep it.'))return;openProject(imported?.content||{html:'',css:'',body:{}},{name:$('start-name').value.trim()||imported?.name||imported?.content.title||'Untitled website',brief:$('start-brief').value.trim()||imported?.brief||'',kind,message:imported?.legacy?'Previous project content imported into the new canvas.':kind==='existing'?'Website opened. Your original file is unchanged.':'Your canvas is ready. Drag in any block to begin.'});autosave();}catch(error){$('start-error').textContent=error.message;}});
...
/* ---------- Autosave (IndexedDB, this browser only) ---------- */
...
function snapshot(){return{format:'oakline-canvas-project',version:2,name:meta.name,brief:meta.brief,kind:meta.kind,html:editor.getHtml(),css:editor.getCss(),savedAt:Date.now()};}
function scheduleAutosave(){clearTimeout(autosaveTimer);autosaveTimer=setTimeout(autosave,700);}
async function autosave(){if(!started)return;try{await idb('readwrite',s=>s.put(snapshot(),KEY));setStatus('Autosaved in this browser · '+new Date().toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}),'saved');}catch{setStatus('Autosave unavailable · download a project to keep your work','pending');}}
let restorable=null;
async function refreshRestore(){try{restorable=await idb('readonly',s=>s.get(KEY));}catch{restorable=null;}const show=!started&&restorable&&typeof restorable.html==='string';$('restore-box').hidden=!show;if(show){const when=new Date(restorable.savedAt);$('restore-meta').textContent=`“${restorable.name||'Untitled website'}” · saved ${when.toLocaleDateString()} ${when.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}`;}}
$('restore-btn').addEventListener('click',()=>{if(!restorable)return;openProject(cleanDocument(restorable.html,restorable.css),{name:String(restorable.name||'Untitled website').slice(0,80),brief:String(restorable.brief||'').slice(0,4000),kind:restorable.kind==='existing'?'existing':'new',message:'Welcome back. Your work is right where you left it.'});setStatus('Restored from this browser','saved');});

/* ---------- Code editor ---------- */
$('edit-code').addEventListener('click',()=>{$('code-html').value=editor.getHtml();$('code-css').value=editor.getCss();$('code-error').textContent='';$('code-dialog').showModal();});
$('apply-code').addEventListener('click',()=>{try{if($('code-html').value.length+$('code-css').value.length>14*1024*1024)throw new Error('Keep the page under 14 MB.');loadContent(cleanDocument($('code-html').value,$('code-css').value));markDirty();$('code-dialog').close();toast('Your HTML and CSS are on the canvas.');}catch(error){$('code-error').textContent=error.message;}});

/* ---------- Export & preview ---------- */
function exportHtml(){const content=cleanDocument(editor.getHtml(),editor.getCss());const attrs=Object.entries(content.body).map(([key,value])=>` ${key}="${esc(value)}"`).join('');return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(meta.name)}</title><meta name="generator" content="Oakline Creator"><style>${BASE_CSS}\n${content.css}</style></head><body${attrs}>${content.html}</body></html>`;}
...
function saveProject(){const file=JSON.stringify(snapshot(),null,2);if(new Blob([file]).size>15*1024*1024){toast('This project exceeds the 15 MB import limit. Remove or resize large images before saving.');return;}download(file,'application/json',slug()+'.oakline.json');dirty=false;toast('Project saved. Open it any time with “Open”.');}
$('download').addEventListener('click',()=>$('export-dialog').showModal());
$('export-html').addEventListener('click',()=>{download(exportHtml(),'text/html;charset=utf-8',slug()+'.html');toast('Your website download has started.');});
$('export-project').addEventListener('click',saveProject);
function openPreview(){$('preview-title').textContent=meta.name;$('preview-frame').srcdoc=exportHtml();$('preview-dialog').showModal();}
...
/* ---------- AI suggestions ---------- */
async function checkAI(){try{const response=await fetch('/api/assistant/status',{cache:'no-store'});if(!response.ok)throw new Error();const data=await response.json();aiReady=data.available===true;$('ai-status').textContent=aiReady?'Ready when you are. Suggestions only change your page when you choose to apply them.':'AI suggestions aren’t connected yet. Every design tool still works.';}catch{$('ai-status').textContent='AI suggestions aren’t connected yet. Every design tool still works.';}finally{$('ask-ai').disabled=!aiReady;}}
